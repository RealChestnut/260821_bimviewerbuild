...
| 프로젝트 저장 형식 (마스터 계획 16절 Follow-up `SQLite 스키마는 별도 ADR`) | 셸이 여닫는 SQLite 파일 하나 `.bim4d`. 모델은 경로와 fingerprint만 적고 원본을 담지 않는다. 경로는 절대와 상대를 함께 적고, 못 찾으면 사용자가 다시 지목한다. 못 찾아도 연결을 지우지 않는다 | `docs/adr/0013-project-store.md` |
